...
//    float springStrengthMult = 0.15f;
    ...
    Vertex beingHoveredOver;//to save having to research for it every time
//
    PeasyCam cam;
        ...
        if (oneOrMoreIsActive) {
            ...
            for (Vertex v : vertices) {
                ...
                if (v.on || v.mouseOver) {
                    ...
                    for (int i = 0; i < v.gettingMoneyFromDisplayIndex; i++) {
                        drawEdges.add(v.gettingMoneyFrom.get(i));
                    }

                    for (int i = 0; i < v.givingMoneyToDisplayIndex; i++) {
                        drawEdges.add(v.givingMoneyTo.get(i));
                    }
...
//                v.mouseOver = false;
        ...
        //there was keyPressed code here. See srcbackups folder to view.

    }

    public void mouseReleased() {

        //if no mouseDrag took place, interpret a click and act accordingly
        if (justAClick) {
...
//                    v.demand *= 1.05;
                    //toggle active state
                    System.out.print("v.on before: " + v.on);
                    v.on = (v.on == true ? false : true);
                    System.out.println(", v.on after: " + v.on);
                }
            }
    ...
    public void keyPressed() {
        ...
        //not yet sure why this reference isn't already sticking
...
//            } else {
//                //hacking now!
//                beingHoveredOver = new Vertex(this, P2D, P2D);
//            }
//        }

        if (key == CODED) {

            //if hovering over a vertex, change its edge view values
            if (keyCode == UP) {
            ...
            for (Vertex v : vertices) {
                if (v.mouseOver) {
                    v.gettingMoneyFromDisplayIndex = 0;
                }
            }
